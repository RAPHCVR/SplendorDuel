...
GameSaveBuilder::GameSaveBuilder() {
    retrevePyramid();
    retrivedeck1();
    retrivedeck2();
    retrivedeck3();
...
void GameSaveBuilder::setPlayers(std::string &player1Name, const Type typep1, std::string &player2Name, const Type typep2) const {
    this->game->setPlayer1(retriveplayer(0));
    this->game->setPlayer2(retriveplayer(1));
    retrivereserve(*game->getPlayer(0),0);
    retrivereserve(*game->getPlayer(1),1);
}

void GameSaveBuilder::roundInit() const {
    game->setRound(retrieveTurn());
}
...
void updateOrAddPlayer(const std::string& playerName) {
    ...
    // Open the database
    if (sqlite3_open(databaseSavePath.c_str(), &db)!= SQLITE_OK) {
        std::cerr << "Error opening database: " << sqlite3_errmsg(db) << std::endl;
        return;
    }
    ...
    // First, try to update the player's score
    std::string updateSql = "UPDATE scores SET score = score + 1 WHERE name = '" + playerName + "';";
    char* errMsg = nullptr;
    if (sqlite3_exec(db, updateSql.c_str(), 0, 0, &errMsg) != SQLITE_OK) {
    ...
    if (sqlite3_changes(db) == 0) {
        ...
        std::string insertSql = "INSERT INTO scores (name, score) VALUES ('" + playerName + "', 1);";
...
std::vector<std::pair<std::string, int>> getPlayersAndScores() {
    ...
    const char* query = "SELECT name, score FROM scores;";
    sqlite3_stmt* stmt;
